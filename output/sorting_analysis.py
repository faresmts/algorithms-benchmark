...
def load_and_process_data():
    """Load and process all sorting benchmark files."""
    dfs = []
    print("Arquivos encontrados:")
    
    # Find all sorting benchmark files
    benchmark_files = list(output_dir.glob('sorting_*_benchmark.csv'))
    ...
    for file in benchmark_files:
        try:
            ...
            # Format: sorting_<test_case>_<size>_benchmark.csv
...
def analyze_comparisons(df):
    ...
    # Extract algorithm name from column (e.g., 'Comparisons Quick Sort' -> 'Quick Sort')
    ...
    # Save to CSV
    comparison_csv_path = output_dir / 'sorting_comparison_counts_summary.csv'
    ...
    for test_case in test_case_order:
        ...
        # Save plot
        plot_path = output_dir / f'sorting_comparisons_{test_case}.png'
...
def analyze_execution_time(df):
    ...
    # Extract algorithm name from column (e.g., 'Execution Time (ms) Quick Sort' -> 'Quick Sort')
    ...
    # Save to CSV
    time_csv_path = output_dir / 'sorting_execution_time_summary.csv'
    ...
    for test_case in test_case_order:
        ...
        # Save plot
        plot_path = output_dir / f'sorting_execution_time_{test_case}.png'
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"  Gráfico salvo em: {plot_path}")
...
def analyze_memory_usage(df):
    ...
    # Save to CSV
    memory_csv_path = output_dir / 'sorting_memory_usage_summary.csv'
    ...
    for test_case in test_case_order:
        ...
        # Save plot
        plot_path = output_dir / f'sorting_memory_usage_{test_case}.png'
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"  Gráfico salvo em: {plot_path}")
...
def create_comparison_table(df):
    ...
    # Round values - comparison counts to integer, others to 4 decimal places
    for col in pivot.columns:
        if col[0] == 'comparison_count':  # Verifica se é uma coluna de contagem de comparações
            pivot[col] = pivot[col].round(0).astype(int)  # Arredonda para inteiro
        else:
            pivot[col] = pivot[col].round(4)  # Mantém 4 casas decimais para outros valores
    ...
    # Save to CSV
    csv_path = output_dir / 'sorting_comparison_table.csv'
    ...
    # Prepare data for table - ensure comparison counts are integers in the display
    table_data = pivot.copy()
    # Ensure comparison counts are displayed as integers (without decimal places)
    for col in table_data.columns:
        if isinstance(col, str) and 'comparison_count' in col:  # Verifica se é uma string contendo 'comparison_count'
    ...
    # Save table as image
    table_path = output_dir / 'sorting_comparison_table.png'
...
def main():
    ...
    if df is not None:
        ...
        analyze_comparisons(df)  # Add comparison analysis
